var Stats=function(){function e(e){return n.appendChild(e.dom),e}
function t(e){for(var t=0;t<n.children.length;t++)
n.children[t].style.display=t===e?"block":"none";l=e}
var l=0,n=document.createElement("div");(n.style.cssText="cursor:pointer;opacity:0"),n.addEventListener("click",function(e){e.preventDefault(),t(++l%n.children.length)},!1);var a=(performance||Date).now(),i=a,o=0,r=e(new Stats.Panel("FPS","#0ff","#002")),f=e(new Stats.Panel("MS","#0f0","#020"));if(self.performance&&self.performance.memory)
var c=e(new Stats.Panel("MB","#f08","#201"));return(t(0),{REVISION:16,domElement:n,addPanel:e,showPanel:t,setMode:t,begin:function(){a=(performance||Date).now()},end:function(){o++;var e=(performance||Date).now();if((f.update(e-a,200),e>i+1e3&&(r.update((1e3*o)/(e-i),100),(i=e),(o=0),c))){var t=performance.memory;c.update(t.usedJSHeapSize/1048576,t.jsHeapSizeLimit/1048576)}
return e},update:function(){a=this.end()},})};(Stats.Panel=function(e,t,l){var n=1/0,a=0,i=Math.round,o=i(window.devicePixelRatio||1),r=80*o,f=48*o,c=3*o,d=2*o,s=3*o,p=15*o,u=74*o,m=30*o,h=document.createElement("canvas");(h.width=r),(h.height=f),(h.style.cssText="width:80px;height:48px");var S=h.getContext("2d");return((S.font="light "+9*o+"px Mulish, sans-serif"),(S.textBaseline="top"),(S.fillStyle=l),S.fillRect(0,0,r,f),(S.fillStyle=t),S.fillText(e,c,d),S.fillRect(s,p,u,m),(S.fillStyle=l),(S.globalAlpha=0.9),S.fillRect(s,p,u,m),{dom:h,update:function(f,v){(n=Math.min(n,f)),(a=Math.max(a,f)),(S.fillStyle=l),(S.globalAlpha=1),S.fillRect(0,0,r,p),(S.fillStyle=t),S.fillText(i(f)+" "+e+" ("+i(n)+"-"+i(a)+")",c,d),S.drawImage(h,s+o,p,u-o,m,s,p,u-o,m),S.fillRect(s+u-o,p,o,m),(S.fillStyle=l),(S.globalAlpha=0.9),S.fillRect(s+u-o,p,o,i((1-f/v)*m))},})}),"object"==typeof module&&(module.exports=Stats);var container=document.querySelector(".globe_container");(function(){"use strict";var root=this;var has_require=typeof require!=="undefined";var THREE=root.THREE||(has_require&&require("three"));if(!THREE)throw new Error("MeshLine requires three.js");function MeshLine(){this.positions=[];this.previous=[];this.next=[];this.side=[];this.width=[];this.indices_array=[];this.uvs=[];this.counters=[];this.geometry=new THREE.BufferGeometry();this.widthCallback=null;this.matrixWorld=new THREE.Matrix4()}
MeshLine.prototype.setMatrixWorld=function(matrixWorld){this.matrixWorld=matrixWorld};MeshLine.prototype.setGeometry=function(g,c){this.widthCallback=c;this.positions=[];this.counters=[];if(g instanceof THREE.Geometry){for(var j=0;j<g.vertices.length;j++){var v=g.vertices[j];var c=j/g.vertices.length;this.positions.push(v.x,v.y,v.z);this.positions.push(v.x,v.y,v.z);this.counters.push(c);this.counters.push(c)}}
...
if(!this.attributes){this.attributes={position:new THREE.BufferAttribute(new Float32Array(this.positions),3),previous:new THREE.BufferAttribute(new Float32Array(this.previous),3),next:new THREE.BufferAttribute(new Float32Array(this.next),3),side:new THREE.BufferAttribute(new Float32Array(this.side),1),width:new THREE.BufferAttribute(new Float32Array(this.width),1),uv:new THREE.BufferAttribute(new Float32Array(this.uvs),2),index:new THREE.BufferAttribute(new Uint16Array(this.indices_array),1),counters:new THREE.BufferAttribute(new Float32Array(this.counters),1),}}else{this.attributes.position.copyArray(new Float32Array(this.positions));this.attributes.position.needsUpdate=!0;this.attributes.previous.copyArray(new Float32Array(this.previous));this.attributes.previous.needsUpdate=!0;this.attributes.next.copyArray(new Float32Array(this.next));this.attributes.next.needsUpdate=!0;this.attributes.side.copyArray(new Float32Array(this.side));this.attributes.side.needsUpdate=!0;this.attributes.width.copyArray(new Float32Array(this.width));this.attributes.width.needsUpdate=!0;this.attributes.uv.copyArray(new Float32Array(this.uvs));this.attributes.uv.needsUpdate=!0;this.attributes.index.copyArray(new Uint16Array(this.indices_array));this.attributes.index.needsUpdate=!0}
...
function mousedown(event){if(_this.enabled===!1)return;event.preventDefault();event.stopPropagation();if(_state===STATE.NONE){_state=event.button}
...
module.perlin2=function(x,y){var X=Math.floor(x),Y=Math.floor(y);x=x-X;y=y-Y;X=X&255;Y=Y&255;var n00=gradP[X+perm[Y]].dot2(x,y);var n01=gradP[X+perm[Y+1]].dot2(x,y-1);var n10=gradP[X+1+perm[Y]].dot2(x-1,y);var n11=gradP[X+1+perm[Y+1]].dot2(x-1,y-1);var u=fade(x);return lerp(lerp(n00,n10,u),lerp(n01,n11,u),fade(y))};module.perlin3=function(x,y,z){var X=Math.floor(x),Y=Math.floor(y),Z=Math.floor(z);x=x-X;y=y-Y;z=z-Z;X=X&255;Y=Y&255;Z=Z&255;var n000=gradP[X+perm[Y+perm[Z]]].dot3(x,y,z);var n001=gradP[X+perm[Y+perm[Z+1]]].dot3(x,y,z-1);var n010=gradP[X+perm[Y+1+perm[Z]]].dot3(x,y-1,z);var n011=gradP[X+perm[Y+1+perm[Z+1]]].dot3(x,y-1,z-1);var n100=gradP[X+1+perm[Y+perm[Z]]].dot3(x-1,y,z);var n101=gradP[X+1+perm[Y+perm[Z+1]]].dot3(x-1,y,z-1);var n110=gradP[X+1+perm[Y+1+perm[Z]]].dot3(x-1,y-1,z);var n111=gradP[X+1+perm[Y+1+perm[Z+1]]].dot3(x-1,y-1,z-1);var u=fade(x);var v=fade(y);var w=fade(z);return lerp(lerp(lerp(n000,n100,u),lerp(n001,n101,u),w),lerp(lerp(n010,n110,u),lerp(n011,n111,u),w),v)}})(this);const SouthAmerica=["Ecuador","Colombia","Paraguay","Uruguay","Guyana","Venezuela, RB","Peru","Panama","Cuba",];const NorthAmerica=["Mexico","United States","Greenland","Iceland"];const Europe=["Norway","Greece","Serbia","Croatia","Spain","Portugal","Germany","Italy","Ukraine","Denmark","Romania",];const Africa=["Chad","Nigeria","Namibia","Zambia","South Sudan","Somalia","Uganda","Kenya","Malawi","Comoros","Madagascar","Ethiopia","Yemen, Rep.","Sudan",];const Asia=["Pakistan","India","Nepal","Kazakhstan","Maldives","Sri Lanka","Mongolia","Thailand","Lao PDR","Cambodia","Vietnam","Singapore","Indonesia",];const Rest=["New Caledonia","New Zealand","Tonga","Fiji","Nauru","Solomon Islands","Kiribati","Tuvalu",];function selectCountries(list,countries){return list.map((name)=>{const country=countries.find((c)=>c.name===name);const{latitude,longitude}=country;return{name,latitude,longitude}})}
const connections={};function getCountry(name,countries){return countries.find((c)=>c.name===name)}
function getCountries(object,countries){return Object.keys(object).reduce((r,e)=>{r[e]=object[e].map((c)=>getCountry(c,countries));return r},{})}
const config={urls:{globeTexture:"../assets/textures/earth_dark.jpg",pointTexture:"../assets/imgs/disc.png",},sizes:{globe:190,globeDotSize:1.2,},scale:{points:0.5,markers:0.5,globeScale:1.5,},rotation:{globe:0.001,},colors:{globeDotColor:"rgb(200,200,200)",globeMarkerColor:"rgb(27, 60, 59)",globeMarkerGlow:"rgb(65,65,65)",globeLines:"rgb(255, 255, 255)",globeLinesDots:"rgb(255, 255, 255)",},display:{points:!0,map:!0,lines:!0,markers:!0,markerLabel:!0,markerPoint:!0,},dots:{total:0,},};const elements={globe:null,atmosphere:null,globePoints:null,lineDots:[],markers:[],markerLabel:[],markerPoint:[],lines:[],};const textures={markerLabels:[],};const groups={map:null,main:null,globe:null,lines:null,points:null,markers:null,atmosphere:null,lineDots:null,};const countries={interval:15000,selected:null,index:0,};const animations={rotateGlobe:!0,};class App{constructor({animate,setup,preload}){this.preload=preload;this.animate=animate;this.setup=setup;window.app=this}
init=async()=>{this.initScene();this.initRenderer();this.initCamera();this.initControls();this.initStats();if(this.preload){await this.preload()}
this.render();this.update()};initScene=()=>{this.scene=new THREE.Scene()};initRenderer=()=>{this.renderer=new THREE.WebGLRenderer({alpha:!0});this.renderer.setClearColor(0x000000,1.0);this.renderer.setPixelRatio(window.devicePixelRatio*1.0);this.renderer.shadowMap.enabled=!0;this.renderer.antialias=!0;if(window.innerWidth>1200){this.renderer.setSize(1000,1000)}else if(window.innerWidth>900){this.renderer.setSize(800,800)}else if(window.innerWidth>600){this.renderer.setSize(600,600)}else if(window.innerWidth>479){this.renderer.setSize(300,300)}else{this.renderer.setSize(200,200)}};initCamera=()=>{this.camera=new THREE.PerspectiveCamera(64,this.ratio,0.1,10000);this.camera.lookAt(this.scene.position);this.camera.position.set(0,15,30)};initControls=()=>{this.controls=new THREE.OrbitControls(this.camera,this.renderer.domElement);this.controls.enableZoom=!1;this.controls.enablePan=!1;this.controls.enableRotate=!0;this.controls.enableDamping=!0;this.controls.dampingFactor=0.05};initStats=()=>{this.stats=new Stats();this.stats.setMode(0);this.stats.domElement.style.position="absolute";this.stats.domElement.style.right="10px";this.stats.domElement.style.bottom="10px";document.body.appendChild(this.stats.domElement)};render=()=>{this.setup(this);container.appendChild(this.renderer.domElement)};update=()=>{this.animate(this);this.stats.update();this.controls.update();this.renderer.render(this.scene,this.camera);requestAnimationFrame(this.update)};addControlGui=(callback)=>{var gui=new dat.GUI();callback(gui)};handleResize=()=>{if(window.innerWidth>1200){this.renderer.setSize(1000,1000)}else if(window.innerWidth>900){this.renderer.setSize(800,800)}else if(window.innerWidth>600){this.renderer.setSize(600,600)}else if(window.innerWidth>479){this.renderer.setSize(300,300)}else{this.renderer.setSize(200,200)}}}
const shaders={atmosphere:{},globe:{},dot:{},};shaders.globe.vertexShader=`
