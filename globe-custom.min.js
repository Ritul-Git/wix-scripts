...
this.render();this.update()};initScene=()=>{this.scene=new THREE.Scene()};initRenderer=()=>{this.renderer=new THREE.WebGLRenderer({alpha:!0});this.renderer.setClearColor(0x000000,1.0);var container_width=document.querySelector(".globe_container").offsetWidth;if(container_width<479){container_width=1000}
this.renderer.setSize(container_width,container_width)};initCamera=()=>{this.camera=new THREE.PerspectiveCamera(64,this.ratio,0.1,10000);this.camera.lookAt(this.scene.position);this.camera.position.set(0,15,30)};initControls=()=>{this.controls=new THREE.OrbitControls(this.camera,this.renderer.domElement);this.controls.enableZoom=!1;this.controls.enablePan=!1;this.controls.enableRotate=!0;this.controls.enableDamping=!0;this.controls.dampingFactor=0.05};initStats=()=>{this.stats=new Stats();this.stats.setMode(0);this.stats.domElement.style.position="absolute";this.stats.domElement.style.right="10px";this.stats.domElement.style.bottom="10px";document.body.appendChild(this.stats.domElement)};render=()=>{this.setup(this);container.appendChild(this.renderer.domElement)};update=()=>{this.animate(this);this.stats.update();this.controls.update();this.renderer.render(this.scene,this.camera);requestAnimationFrame(this.update)};addControlGui=(callback)=>{var gui=new dat.GUI();callback(gui)};handleResize=()=>{var container_width=document.querySelector(".globe_container").offsetWidth;if(container_width<479){container_width=1000}
this.renderer.setSize(container_width,container_width)}}
...
createText(){const element=document.createElement("canvas");const canvas=new fabric.Canvas(element);console.log(window.innerWidth);var text=new fabric.Text(this.labelText,{left:0,top:0,fill:this.textColor,fontFamily:"Mulish",});canvas.add(text);return element}}
